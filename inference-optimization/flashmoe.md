...
![Figure 1: Illustration of an MoE model decoder layer](images/flashmoe/x1.png)
...
![Figure 2: Heatmap comparing LRU and Belady cache policies](images/flashmoe/x2.png)

*图 2：LRU 与 Belady 缓存策略的热力图对比。最深的颜色表示专家在该时间步被路由，颜色随时间向后变浅。*
...
![Figure 4: Overall training method illustration of FlashMoE](images/flashmoe/x5.png)

*图 4：FlashMoE 整体训练方法示意图*

### 2.3 System Architecture / 系统架构

![Figure 3: (a) Overall system of FlashMoE. (b) FlashMoE's prefill process](images/flashmoe/x3.png)

*图 3：(a) FlashMoE 整体系统架构。(b) FlashMoE 的预填充过程。*
...
![Figure 5: Decoder layer computation pipeline of FlashMoE](images/flashmoe/x6.png)

*图 5：FlashMoE 解码器层计算流水线*
...
![Figure 6: Hit rate by cache size and replacement policy](images/flashmoe/x7.png)

*图 6：不同缓存大小与替换策略的命中率。(a) OLMoE-1B-7B，(b) Qwen3-30B-A3B。*
...
![Figure 7: Latency and speed benchmarks across systems and policies](images/flashmoe/x9.png)

*图 7：(a,d) MoE 推理系统的模型加载与预填充延迟。(b,e) 不同缓存替换策略下的 token 生成速度。(c,f) 不同推理系统的 token 生成速度。*
